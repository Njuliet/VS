//#include <pcl/registration/ia_ransac.h>//暂时没跑，这里边有滤波、法线和粗配准、精配准的代码，目前跑ICP.cpp已经成功
//https://blog.csdn.net/qq_33933704/article/details/78653289
...
//	pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> src_h(pcd_src, 0, 255, 0);//绿色  RGB
//	pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> tgt_h(pcd_tgt, 255, 0, 0);//红色
//	pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> final_h(pcd_final, 0, 0, 255);//蓝色
...
//	pcl::io::loadPCDFile("haihang_nose.pcd", *cloud_src_o);
//	PointCloud::Ptr cloud_tgt_o(new PointCloud);//目标点云
//	pcl::io::loadPCDFile("linlin_nose.pcd", *cloud_tgt_o);
//
//	clock_t start = clock();
//	//去除NAN点
//	std::vector<int> indices_src; //保存去除的点的索引
//	pcl::removeNaNFromPointCloud(*cloud_src_o, *cloud_src_o, indices_src);//在使用PCL进行数据处理时候，很多算法都会考虑无效点,其主要是通过判断PointCloud类里的数据成员是否包含NaN。
