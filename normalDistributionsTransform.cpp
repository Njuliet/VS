...
main(int argc, char** argv)
{
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZ>("dragonUpRight_0.pcd", *target_cloud) == -1)
	{
		PCL_ERROR("Couldn't read file dragonUpRight_0.pcd \n");
		return (-1);
	}
	std::cout << "Loaded " << target_cloud->size() << " data points from dragonUpRight_0.pcd" << std::endl;

	// 加载从新视角得到的第二次扫描点云数据作为源点云
	pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud(new pcl::PointCloud<pcl::PointXYZ>);
	if (pcl::io::loadPCDFile<pcl::PointXYZ>("dragonUpRight_24.pcd", *input_cloud) == -1)
	{
		PCL_ERROR("Couldn't read file dragonUpRight_24.pcd \n");
		return (-1);
	}
	std::cout << "Loaded " << input_cloud->size() << " data points from dragonUpRight_24.pcd" << std::endl;

		//去除NAN点   去除无效点之后程序不再报错
		std::vector<int> indices_src; //保存去除的点的索引
		pcl::removeNaNFromPointCloud(*target_cloud, *target_cloud, indices_src);//在使用PCL进行数据处理时候，很多算法都会考虑无效点,其主要是通过判断PointCloud类里的数据成员是否包含NaN。
		std::cout << "remove *target_cloud nan" << "in " << time.toc() << " ms\n" << " valid points have " << target_cloud->size() << endl;
		
		std::vector<int> indices_src1; 
		pcl::removeNaNFromPointCloud(*input_cloud, *input_cloud, indices_src1);//
		std::cout << "remove *input_cloud nan" << "in " << time.toc() << " ms\n" << " valid points have " << input_cloud->size() << endl;
	...
	std::cout << "Filtered cloud contains " << filtered_cloud->size()
		<< " data points from dragonUpRight_24.pcd" << std::endl;
	...
	// 保存转换后的源点云作为最终的变换输出
	pcl::io::savePCDFileASCII("dragonUpRight_transformed_NDT.pcd", *output_cloud);
